...
foreach ($row as $key => $value) {
	if($value[0]){
		$sql = 'INSERT INTO `library`.`book` (`bookID`, `categoryID`, `title`, `press`, `year`, `author`, `price`, `stock`, `total`) VALUES (NULL, ' . $value[0] . ', "' . $value[1] . '", "' . $value[2] . '", ' . $value[3] . ', "' . $value[4] . '", ' . $value[5] . ', ' . $value[6] . ', ' . $value[7] . ')';
		$conn->query($sql);
		if($conn->sqlstate == 0){
